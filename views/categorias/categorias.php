...
<head>
  ...
  <title>mi_bodega - Categorías</title>
  ...
  <main class="flex-1 p-6 space-y-6 max-w-4xl mx-auto">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-xs text-gray-500">Administra las familias de productos</p>
      </div>
    </div>
    ...
    <div class="bg-white p-5 rounded-xl border border-gray-200 shadow-sm space-y-4">
      <h3 class="font-bold text-gray-900 text-sm">Nueva Categoría</h3>
      ...
      <form class="flex gap-3" action="index.php?controller=categoriasController&action=crear" method="POST">
        <input type="text" name="nombre" placeholder="Nombre de la categoría" class="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-xs focus:outline-none focus:border-olive">
        ...
        <tbody class="divide-y divide-gray-100">
          <?php foreach ($categorias as $categoria): ?>
            <tr class="hover:bg-gray-50">
              <td class="p-3 font-medium text-gray-900"> <?= $categoria['categorias_nombre'] ?></td>
              <td class="p-3 text-right">
                <a href="index.php?controller=categoriasController&action=editar&id=<?= $categoria['categorias_id'] ?>" class="inline-block p-1.5 text-gray-500 hover:text-olive hover:bg-gray-100 rounded-md transition-colors">
