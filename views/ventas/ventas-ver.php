...
<body class="bg-warmBg text-gray-800 font-sans min-h-screen flex">
  ...
  <main class="flex-1 p-6 space-y-6 max-w-4xl mx-auto">
    <div class="flex items-center space-x-3">
      ...
      <div>
        <h2 class="font-display text-3xl font-semibold tracking-[-0.015em] text-gray-900">Detalle de Venta #<?= $venta['venta_id'] ?></h2>
        <p class="text-sm text-gray-500">Información completa de la venta</p>
      </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <?php if ($venta['venta_numero_pago']): ?>
          <div>
            <p class="text-sm text-gray-500">Número de Referencia</p>
    ...
    <div class="flex justify-end space-x-3">
      <a href="index.php?controller=ventasController&action=listar" class="px-4 py-2 border border-gray-300 rounded-lg text-sm font-semibold text-gray-600 hover:bg-gray-50 transition-colors">
        Volver al Listado
      </a>
      <?php if ($venta['venta_estado'] == 'pendiente'): ?>
        <a href="index.php?controller=ventasController&action=editar&id=<?= $venta['venta_id'] ?>" class="px-4 py-2 bg-olive hover:bg-olive-hover text-white text-sm font-bold rounded-lg transition-colors">
          Completar Venta
        </a>
      <?php endif; ?>
    </div>
